	...
	public static void main(String[] args) throws IOException {
		...
		try {
			while (true) {
				...
				if (clientNumber < Number_Of_Clients) {
					Socket soc = listener.accept();
					InetAddress addr = soc.getInetAddress();
					log("ip address of client is: "+ addr.getHostAddress());
					log("Connected to: " + clientNumber );
					new Thread(new MultithreadingServer(soc , clientNumber++, defaultpath).run());
